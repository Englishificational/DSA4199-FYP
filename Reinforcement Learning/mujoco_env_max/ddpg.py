...
from torch.optim import SGLMLD
...
class DDPG:
    def __init__(self, beta, epsilon, learning_rate, gamma, tau, hidden_size_dim0, hidden_size_dim1, num_inputs, action_space, train_mode, alpha, replay_size,
        ...
        if self.train_mode:
            ...
            elif(optimizer == 'SGLMLD'):
                self.actor_optim = SGLMLD(self.actor.parameters(), lr=1e-4, noise=epsilon, alpha=0.999)
            ...
            if(optimizer == 'SGLD'):
                ...
                self.adversary_optim = SGLMLD(self.adversary.parameters(), lr=1e-4, noise=epsilon, alpha=0.999)
            elif(optimizer == 'RMSprop'):
    ...
    def select_action(self, state, action_noise=None, param_noise=None, mdp_type='mdp'):
        ...
        if mdp_type != 'mdp':
            ...
            if((self.optimizer == 'SGLD' or self.optimizer == 'SGLMLD') and self.two_player):
    ...
    def update_robust_non_flip(self, state_batch, action_batch, reward_batch, mask_batch, next_state_batch,
                      mdp_type, robust_update_type, policy_loss_min=0, adversary_loss_max=0):
        #print("here c_diff:",c_difference)
        # TRAIN CRITIC
        #print("~~~ here 0")
        ...
        adversary_loss = adversary_loss.item() # i think this can safely be moved here?
        c_difference = max(adversary_loss_max - adversary_loss, 0)
        if c_difference:
            self.adversary_optim.step(c_difference=c_difference)
        else:
            self.adversary_optim.step()
        #self.adversary_optim.step() 
        #print("adversary loss 3:", adversary_loss)
        ...
        policy_loss = policy_loss.item() # i think this can safely be moved here?
        # if policy_loss > -10:
        #     print("policy_loss: ", policy_loss)
        #     print("policy_loss_min: ", policy_loss_min)
        #     print("==")
        c_difference = max(policy_loss - policy_loss_min, 0)
        c_difference = 0 # only do for adversary for here 
        if c_difference:
            self.actor_optim.step(c_difference=c_difference)
        else:
    ...
    def update_robust_flip(self, state_batch, action_batch, reward_batch, mask_batch, next_state_batch, adversary_update,
                      mdp_type, robust_update_type, c_difference=0):
    ...
    def update_non_robust(self, state_batch, action_batch, reward_batch, mask_batch, next_state_batch, c_difference=0):
        #print("here c_diff:",c_difference)
        print("not using this")
    ...
    def update_parameters(self, batch_size, sgld_outer_update, mdp_type='mdp', exploration_method='mdp',
                            policy_loss_min=0, adversary_loss_max=0):
        c_difference = 0
        ...
        #print("robust_update_type: ", robust_update_type)
        ...
        if robust_update_type is not None:
            ...
            _value_loss, _policy_loss, _adversary_loss = self.update_robust_non_flip(state_batch, action_batch, reward_batch,
                                                                            ...
                                                                            adversary_loss_max=adversary_loss_max)
        ...
        self.soft_update()
        if(sgld_outer_update and (self.optimizer == 'SGLD' or self.optimizer == 'SGLMLD') and self.two_player):
